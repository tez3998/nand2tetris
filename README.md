...
結論を言うと、おそらく、アスタリスクの意味は「アスタリスク以下をRAMのアドレスとして扱え」というものだと思う。

どういうことかと言うと、書籍に出てくる次の疑似コードは
```
*ARG = pop()
```
次のようなコードと同じ意味だと思う。
...
RAM[ARG] = pop()
// 日本語の意味：
// スタックからpopしてきた値をRAMのARG番地、つまりRAM[2]番地にセットする
//（ARGはRAM[2]に対応する。7章のレジスタとシンボルの対応表を参照）
```

一方、アスタリスクの付いていない例を見ると、例えば、書籍に出てくる次のコードであれば、
```
SP = ARG+1
```
というものがあるが、これはARG（実態はRAM[2]）に1を足した値をSP（実態はRAM[0]）にセットするという意味となる。

「アスタリスクに遭遇したら、`*アスタリスク以下`を`RAM[アスタリスク以下]`と脳内で変換すればよい」と考えると楽かもしれない。
